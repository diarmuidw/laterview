...
connect ('imagetest',host='xxxxxxx', port =xxxxx, username='xxxxx', password = 'xxxxxx')

def upload(file, username):
    try:
        log.info("starting Upload %s"%file)
        conn = S3Connection('xxxx', 'xxxxx')
        ...
        k = Key(bucket)
        k.key = '%s/%s/%s/%s'%(dt,username,hm,uuid.uuid4())
        k.set_contents_from_filename(file)
        k.set_acl('public-read')
        
        #print "writing to s3:", file
        ...
        i = Image(camera = cams[0], camname = cams[0].name, day = dt, hour = nowdt.strftime('%H'), minute = nowdt.strftime('%M'),  key=k.key, format = 'jpg')
        i.save()
        log.info( 'Saved image info to mongo with key %s'%i.key)
        try:
        ...
        log.info( 'Saved image data to mongo with key %s'%i.key)
